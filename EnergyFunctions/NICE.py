...
class CNN(nn.Module):
    features: int = 10  # Number of output classes, e.g., 10 for MNIST
...
def load_model_nice(dataset: str, dim: int):
    ...
    pickle_file = dir_path + "/NiceUtils/checkpoints/" + f"params_nice_{dataset}_{im_size}x{im_size}_flax.pkl"
...
class NiceTarget(EnergyModelClass):
    def __init__(self, config, dataset="mnist", log_Z=0., can_sample=True, sample_bounds=None) -> None:
        #super().__init__(dim, log_Z, can_sample)
        config["dim_x"] = 14 * 14  # only implemented for mnist
        super().__init__(config)
        self.data_ndim = config["dim_x"]
        self.dim = config["dim_x"] # only implemented for mnist
        self.im_size = int(np.sqrt(self.dim))

        self.logpx_fn_without_rng, _, self.sample_fn_clean = load_model_nice(dataset, self.dim)

        dir_path = os.path.dirname(os.path.realpath(__file__))
        
        pickle_path = dir_path + "/NiceUtils/checkpoints/" + f"params_nice_{dataset}_{self.im_size}x{self.im_size}_flax.pkl"

        # if not os.path.exists(pickle_path):
        #     raise FileNotFoundError(f"Model parameters not found at {pickle_path}. Please ensure the model is trained and the parameters are saved.")   
        # with open(pickle_path, "rb") as f:
        #     state = pickle.load(f)


        state = checkpoints.restore_checkpoint(ckpt_dir=dir_path + "/NiceUtils", target=None,
                                               prefix="{}_{}x{}_classifier_checkpoint".format(dataset,
                                                                                              int(math.sqrt(self.dim)),
                                                                                              int(math.sqrt(self.dim))))
        if dataset == "mnist":
            classifier = CNN()
        else:
        ...
        self.classify = jax.jit(functools.partial(classify, cnn=classifier, params=state['params'], im_size=self.im_size))
        self.has_tractable_distribution = True

    def get_dim(self):
        return self.dim
    
    def energy_function(self, x: chex.Array) -> chex.Array:
    ...
    def compute_emc(self, samples: chex.Array = None):
...
if __name__ == '__main__':
    ...
    nice = NiceTarget({"dim_x": 14 * 14, "scaling": 1.0, "name": "NICE"})
    key = jax.random.PRNGKey(0)
    n = 1000
    samples = nice.sample(key, (n,))
    print(samples.shape)
    print(np.max(samples), np.min(samples), np.mean(samples), np.mean(np.std(samples, axis = 0)))

    # Plot histogram of grayscale values
    plt.figure(figsize=(10, 6))
    pixel_values = samples.flatten()
    plt.hist(pixel_values, bins=50, color='skyblue', edgecolor='black', alpha=0.7)
    plt.title('Histogram of Grayscale Pixel Values')
    plt.xlabel('Pixel Value')
    plt.ylabel('Frequency')
    plt.grid(alpha=0.3)
    plt.savefig("nice_histogram.png", dpi=300, bbox_inches='tight')
    plt.show()
    # Plot the samples
    ...
    for i in range(n):
        img = samples[i].reshape(14, 14)
        axes[i].imshow(img, cmap='gray')
        axes[i].axis('off')


    plt.tight_layout()
    plt.savefig("nice_samples.png", dpi=300, bbox_inches='tight')
    plt.show()
