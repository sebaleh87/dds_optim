...
class DoubleMoonClass(EnergyModelClass):
    def __init__(self, config ):
        self.data_z, self.labels = self.create_double_moon_dataset(1000)
        self.z_dim = 2
    ...
    def calc_energy(self, diff_samples, energy_params, key):
        ...
        param_energy = jnp.sum(diff_samples**2)
    ...
    def scale_samples(self, diff_samples, energy_params, key):
        ...
        Y_pred = self.vmap_apply(diff_params, z)
    ...
    def energy_function(self, Y_pred_Y_target):
        ...
        CrossEntropy = lambda log_Y_pred, y_target: -jnp.mean(y_target*log_Y_pred[:,0] + (1-y_target)*log_Y_pred[:,1])
    ...
    def visualize_models(self, diff_samples):
        ...
        for idx, diff_sample in enumerate(diff_samples):
            ...
            Y_pred = self.vmap_apply(diff_params, full_z_samples)
            
            #fig, axs = plt.subplots(idx + 1, 2, figsize=(12, 6))

            scatter1 = axs[counter].scatter(full_z_samples[:, 0], full_z_samples[:, 1], c=Y_pred.argmax(axis=-1), cmap='viridis')
            axs[counter].set_title('Predicted Labels')
            fig.colorbar(scatter1, ax=axs[counter])
            counter += 1

            scatter2 = axs[counter].scatter(full_z_samples[:, 0], full_z_samples[:, 1], c=self.labels.flatten(), cmap='viridis')
